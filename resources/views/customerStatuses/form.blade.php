<x-app-layout>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('customer.statuses.index') }}" class="btn btn-primary shadow-md mr-2">Voltar</a>
        </div>
        <div class="intro-y col-span-12">
            @if (empty($customerStatus->id))
                {!! Form::open([
                    'route' => 'customer.statuses.store',
                    'method' => 'post',
                    'class' => 'needs-validation',
                ]) !!}
            @else
                {!! Form::model($customerStatus, [
                    'route' => ['customer.statuses.update', $customerStatus->id],
                    'method' => 'put',
                    'class' => 'needs-validation',
                ]) !!}
            @endif
